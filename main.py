...
async def crawl_data_tapfiliate(inputURL, username, password):
    # Edit url
    last_string = inputURL[-1]
    # print(last_string)
    if last_string == "/":
        inputURL = inputURL[:-1]
    # print(inputURL)
    async with aiohttp.ClientSession() as session:
        # print("\nRequest Login\n")
        url = f"{inputURL}/login"
        ssid = ''

        async with session.get(url, headers=default_headers) as res:
            if res.status == 200:
        ...
        url = f"{inputURL}/_form_params/"
        token = ''
        isCaptcha = False
        async with session.get(url, headers=default_headers) as res_form:
            if res.status == 200:
                # print(res_form.status)
                # print(res_form.text)
        ...
        if isCaptcha is False:
            ...
            payload = {
                ...
                "_username": username,
                "_password": password,
                "g-recaptcha-response": await solve_captcha(inputURL),
            }

        url = f"{inputURL}/login_check"
        headers = default_headers
        headers['Cookie'] = f'TAPSESSID={ssid}'
        ...
        async with session.post(url, headers=headers, data=payload) as res_login:
            ...
            if "Dashboard | " in isLoginSuccess:
                ...
                url = f"{inputURL}/api-stateful/pi/reports-depr/date/?date_from={date_30_days_ago_str}&date_to={current_date_str}&sort_by=title&sort_direction=DESC&page=1"
                ...
                return f"Error with login_check, {inputURL}"


async def crawl_data_post_affiliate(input_url, username, password):
    idx = input_url.index("/affiliates")
    input_url = input_url[:idx]
    ...
    async with aiohttp.ClientSession() as session:
        url = f"{input_url}/affiliates/login.php"
        response0 = await session.get(url, headers=default_headers)
        ...
        # print("print Signup pap sid")
        # print(signup_pap_sid)
        ...
        url_post = f"{input_url}/scripts/server.php"
        response0 = await session.post(url_post, data=form_data, headers=headers)

        # print("Request server to login 1 \n")
        ...
        json_data = {
            ...
            "requests": [
                {"C": "Gpf_Auth_Service", "M": "authenticate", "fields": [["name", "value"], ["Id", ""],
        ...
        # print("Request server to Login with user , pwd\n")
        
        isSuccess = await response0.json()
        # print(isSuccess[0]['message'])
        # print(response0.cookies)
        # print(response0.headers)
        ...
        # print(response_html_1.status)
        # print(response_html_1.text)
...
async def crawl_data_affiliatly(inputURL, username, password):

    async def crawl_data_html(html_content):
        soup = BeautifulSoup(html_content, 'html.parser')
    ...
    async with aiohttp.ClientSession() as session:
        url = f"{inputURL}/af-1040475/affiliate.panel"
        ...
        async with session.post(url, headers=default_headers, data=payload) as response:
            ...
            if _affiliatly_logged is not None:
                ...
                async with session.get(url, headers=headers) as response:
                    return await crawl_data_html(await response.text())
            else:
                return f"Error with login website , {inputURL}"


async def crawlDataControl(args):
    loginRequestUrl, email, password = args

    if 'withblaze' in loginRequestUrl or 'flocksocial' in loginRequestUrl:
        return await crawl_data_tapfiliate(loginRequestUrl, email, password)
    elif 'describely' in loginRequestUrl or 'postaffiliatepro' in loginRequestUrl:
        return await crawl_data_post_affiliate(loginRequestUrl, email, password)
    elif 'affiliatly.com' in loginRequestUrl:
        return await crawl_data_affiliatly(loginRequestUrl, email, password)
...
async def main():
    ...
    data = await pl.task.map(crawlDataControl, data, workers=100)
    for item in data:
        print(item)
